...
| 司机位置标记 | Marker | GPS | 实时更新，随司机位置移动 |
| 当前订单卡片 | Card | DriverOrderList | **仅显示当前进行中的一条订单**（单例）；有订单时展示，无订单时隐藏 |
| 上线/下线开关 | Toggle | DriverOnline/Offline | 大按钮，居中底部 |
| 开始/停止接单 | Toggle | StartAcceptOrder/StopAcceptOrder | 上线后可见 |
| 抢单弹窗触发 | Modal | WebSocket 推送 | 收到订单推荐时弹出（见 §4.3） |
| 导航栏 | NavBar | 静态 | 左侧:消息图标(预留) 右侧:个人中心 |

> **注意**：当前 proto 中没有订单推荐推送的 RPC 接口。建议前端通过 **WebSocket 长连接**接收后端推送的待抢单数据，收到后弹出抢单窗口。`AcceptOrder` 作为抢单确认的回调接口。
...
> 从首页（P02）地图模式下，当 WebSocket 推送新订单推荐时弹出。

#### 页面元素

| 元素 | 组件 | 数据来源 | 说明 |
|------|------|----------|------|
| 乘客上车点 | 地址文字 + 地图小图 | WebSocket 推送数据 | 显示起点名及位置标注 |
| 乘客目的地 | 地址文字 | WebSocket 推送数据 | 显示终点名 |
| 距您距离 | 文字 + 数值 | WebSocket 推送数据 | 显示司机到上车点的距离（米） |
| 预估价格 | 文字 | WebSocket 推送数据 | 显示金额 |
| 接单按钮 | Button | → AcceptOrder | 大按钮 + 倒计时（建议 10-15s） |
| 跳过按钮 | Button | 纯前端 | 关闭弹窗，回到听单状态 |

#### 状态说明

- **等待态**：弹窗出现，倒计时开始，显示订单信息等待司机决策
- **抢单中**：点击接单 → 按钮 Loading，调用 AcceptOrder
- **抢单成功**：关闭弹窗，进入"去接乘客"状态
- **抢单失败（已被抢）**：Toast "订单已被其他司机接走" + 关闭弹窗
- **倒计时结束**：无操作 → 自动关闭弹窗，回到听单

#### 交互流程

```
WebSocket 推送 → 弹窗出现 + 倒计时 → 
  ├─ 点击"接单" → AcceptOrder(携带 antiFraudToken) 
  │     ├─ success=true → 进入行程
  │     └─ success=false → Toast 失败原因
  └─ 倒计时结束/点击"跳过" → 关闭弹窗，等待下一单
```
...
> 该页数据主要来源于 `DriverOrderList`（当前订单）接口。注意：该接口当前只返回一条进行中的订单（单例），按单条订单展示设计即可。
...
> 当司机点击"我已到达"后进入此状态，等待乘客上车。

#### 页面元素

| 元素 | 数据来源 | 说明 |
|------|----------|------|
| 等待状态 | 本地 | 显示"已到达上车点，等待乘客" |
| 乘客信息卡片 | DriverOrderList | 头像、姓名(脱敏)、电话 |
| 乘客确认码输入框 | Input(4位数字) | 乘客手机端显示，司机输入后 → StartTrip |
| 开始行程按钮 | Button → StartTrip | 输入确认码后启用 |
| 联系乘客 | 呼叫/IM | 乘客未出现时可联系 |
| 乘客未出现 | 倒计时 | 等待超时后 → 允许取消订单（免责） |
...
到达乘客位置 → 点击"我已到达" →
进入等待状态 →
  情况A: 乘客上车 → 出示确认码 → 司机输入 4位码 → 点击"开始行程" → StartTrip
  情况B: 乘客未出现 → 等待5分钟后 → 可免责取消订单 → CancelOrder
```
...
> 乘客已上车，司机正在导航前往目的地。**注意**：当前 proto 中没有 `EndTrip` 接口，行程结束功能需后端补充，前端设计时应预留交互位置。
...
| 附加信息 | 投诉状态、乘客标签 | complaintStatus, passengerTags |
...
| 区域 | 内容 | 数据来源 |
|------|------|----------|
| 顶部状态区 | "已取消" + 责任方标签 | status + cancelResponsibility |
| 行程路线 | 起点→终点 | startAddress → endAddress |
| 乘客信息 | 头像、姓名、电话 | passengerName, passengerPhone, passengerAvatar |
| 时间线 | 接单时间→取消时间 | bookTime → cancelTime |
| 费用信息 | 取消费用（如有）、优惠券抵扣 | cancelFee, couponDeduction |
| 取消原因 | 司机选择的取消原因 + 责任方 | cancelReason, cancelResponsibility |
| 申诉入口 | "去申诉"按钮（`appealSupport=true` 时显示） | CancelOrderResp.appealSupport |
...
| 卡片内容 | — | 起点→终点、金额、时间、状态标签、服务分（`serviceScore`） |
...
| 统计信息 | 服务分、订单数 | 只读展示 |
| 功能列表 | 历史订单 / 设置页(P13) | 点击跳转 |
| 退出登录 | 退出按钮 | → OutLogin → 回到登录页 |

---

### 4.11 P11/P12 — 认证流程页（实名认证 + 车辆认证）
...
graph TD
    ...
    L -->|到达目的地<br>EndTrip(需补充)| N[行程结束-待支付]
    N -->|支付完成| O[订单完成]
    L -->|取消订单| M
